        ...
        <div className="hlp-wrapper">
            ...
            <section className="hlp-hero" style={{ position: 'relative', overflow: 'hidden', padding: '12rem 0 8rem' }}>
                <div className="hlp-hero__bg-effects">
                    <div className="hlp-blob hlp-blob--1" style={{ background: 'linear-gradient(135deg, rgba(60,149,187,0.25), rgba(97,174,207,0.12))', filter: 'blur(80px)', width: '600px', height: '600px', position: 'absolute', top: '-100px', right: '-100px', borderRadius: '50%', zIndex: 0 }}></div>
                    <div className="hlp-blob hlp-blob--2" style={{ background: 'linear-gradient(135deg, rgba(152,202,225,0.3), rgba(204,228,240,0.1))', filter: 'blur(100px)', width: '500px', height: '500px', position: 'absolute', bottom: '-100px', left: '-100px', borderRadius: '50%', zIndex: 0 }}></div>
                </div>

                <div className="hlp-container hlp-hero__content reveal" style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
                    <div className="hlp-hero__badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(60, 149, 187, 0.1)', color: '#3c95bb', padding: '0.6rem 1.25rem', borderRadius: '50px', fontSize: '0.9rem', fontWeight: 700, marginBottom: '2rem', border: '1px solid rgba(60, 149, 187, 0.2)', boxShadow: '0 4px 15px rgba(60, 149, 187, 0.1)' }}>
                        <Zap size={16} className="hlp-pulse-icon" style={{ animation: 'pulse 2s infinite' }} />
                        <span>Новая эра доступности с Грок ИИ</span>
                    </div>

                    <h1 className="hlp-hero__title" style={{ fontSize: 'clamp(3rem, 8vw, 5.5rem)', fontWeight: 900, lineHeight: 1.1, letterSpacing: '-0.04em', marginBottom: '1.5rem', color: '#214559' }}>
                        Слышать мир<br /><span className="hlp-text-gradient" style={{ background: 'linear-gradient(135deg, #3c95bb, #61aecf)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>через интерфейс</span>
                    </h1>
                    ...
                    <div className="hlp-hero__actions" style={{ display: 'flex', gap: '1.25rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                        <button onClick={goRegister} className="hlp-btn-primary hlp-btn--xl" style={{ padding: '1.1rem 2.5rem', borderRadius: '16px', background: 'linear-gradient(135deg, #3c95bb, #61aecf)', color: '#fff', border: 'none', fontWeight: 700, fontSize: '1.1rem', cursor: 'pointer', boxShadow: '0 10px 30px rgba(60, 149, 187, 0.3)', display: 'flex', alignItems: 'center', gap: '0.75rem', transition: 'transform 0.2s' }}>
                            Начать бесплатно <ChevronRight size={20} />
                        </button>
                        <button onClick={() => scrollTo('features')} className="hlp-btn-secondary hlp-btn--xl" style={{ padding: '1.1rem 2.5rem', borderRadius: '16px', background: '#fff', color: '#214559', border: '1px solid rgba(60,149,187,0.2)', fontWeight: 700, fontSize: '1.1rem', cursor: 'pointer', boxShadow: '0 10px 30px rgba(0,0,0,0.03)', display: 'flex', alignItems: 'center', gap: '0.75rem', transition: 'all 0.2s' }}>
                            Посмотреть демо
                        </button>
                    </div>
                    ...
                    <div className="hlp-hero__visual reveal" style={{ marginTop: '5rem', position: 'relative' }}>
                        ...
                        <div className="hlp-wave-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', height: '120px' }}>
                            {[...Array(30)].map((_, i) => (
                                <div key={i} className="hlp-wave-bar" style={{ width: '6px', background: 'linear-gradient(to top, #3c95bb, #61aecf)', borderRadius: '10px', height: `${10 + Math.random() * 90}%`, animation: `wave 1.5s ease-in-out infinite alternate`, animationDelay: `${i * 0.05}s` }}></div>
                            ))}
                        </div>
                    </div>
                </div>
            ...
            <section id="features" className="hlp-section">
                <div className="hlp-container">
                    ...
                    <div className="hlp-features-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem', marginTop: '4rem' }}>
                        {FEATURES.map((f, i) => (
                            <div key={i} className="hlp-feature-card reveal" style={{ background: '#fff', padding: '2.5rem', borderRadius: '28px', border: '1px solid rgba(60,149,187,0.15)', boxShadow: '0 10px 40px rgba(0,0,0,0.03)', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', position: 'relative', overflow: 'hidden', transitionDelay: `${i * 0.1}s` }}>
                                <div className="hlp-feature-card__badge" style={{ position: 'absolute', top: '1.5rem', right: '1.5rem', background: '#e6f1f8', padding: '0.4rem 0.8rem', borderRadius: '50px', fontSize: '0.75rem', fontWeight: 700, color: '#3c95bb', border: '1px solid rgba(60,149,187,0.2)' }}>{f.badge}</div>
                                <div className="hlp-feature-card__icon" style={{ width: '56px', height: '56px', background: '#e6f1f8', color: '#3c95bb', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>{f.icon}</div>
                                <h3 className="hlp-feature-card__title" style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '1rem', color: '#214559' }}>{f.title}</h3>
                                <p className="hlp-feature-card__desc" style={{ color: '#22526a', lineHeight: 1.6, marginBottom: '1.5rem' }}>{f.desc}</p>
